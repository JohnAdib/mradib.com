...
export function DesktopNavigation(
	...
	return (
		<nav {...props}>
			<ul className="flex rounded-full bg-surface/90 px-3 text-sm font-medium text-zinc-800 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur-sm dark:bg-zinc-800/90 dark:text-zinc-200 dark:ring-white/10">
				{mainNav.map((item) => (
					<NavItem
						key={item.href}
						href={item.href}
