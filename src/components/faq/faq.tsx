...
export default function Faq({ list }: { list?: IFaqQA[] }) {
  if (!list) {
    return null;
  }
      ...
      <div className="lg:grid lg:grid-cols-12 lg:gap-8">
        <div className="lg:col-span-5">
          ...
          <p className="mt-4 text-base leading-7 text-slate-500 dark:text-slate-300">
            سوال بدون جواب برات باقی مونده؟ پرسش و پاسخ رو چک کن. اگه جواب سوالت
            رو بازم پیدا نکردی،{" "}
            <a href="/fa/contact" className="font-semibold">
              با من تماس بگیر
            </a>
            .
          </p>
          ...
          <div className="space-y-10">
            {list?.map((faq) => (
              <div id={"faq-" + faq.id} key={faq.id}>
                ...
                <p className="mt-2 text-sm leading-7 text-slate-00 dark:text-slate-300">
                  {faq.a}
                </p>
              </div>
            ))}
